...
// ---- boot ----------------------------------------------------------
// the canvas marquee/banner draw in the kit font; ask for it up front so it is
// ready by the time they render (canvas never triggers a webfont load itself)
void document.fonts?.load('800 26px "Kenney Future"');
document.querySelectorAll<HTMLImageElement>("img[data-icon]").forEach((img) => {
  img.src = ICON[img.dataset.icon as IconName];
});
...
function buildModes(): void {
  ...
  for (const id of MODE_ORDER) {
    ...
    btn.innerHTML =
      ...
      `<div class="m-name">${m.label.split(" ")[0]}</div>` +
      `<div class="m-odds">${Math.round(m.grab * 100)}% grab<br/>up to ${maxWinX(m).toFixed(1)}×</div>`;
...
function setModeTheme(id: ModeId): void {
...
function tile(value: string, label: string, opts: { hi?: boolean; small?: boolean; icon?: IconName } = {}): string {
  const cls = ["stat-tile", opts.hi ? "hi" : "", opts.small ? "small" : ""].filter(Boolean).join(" ");
  const ic = opts.icon ? `<img class="stat-icon" src="${ICON[opts.icon]}" alt="" />` : "";
  return `<div class="${cls}"><div class="stat-head">${ic}${label}</div><div class="stat-value">${value}</div></div>`;
}
...
function updateOdds(): void {
  ...
  $("odds").innerHTML =
    ...
    `<div class="stat-grid">` +
    tile(`${(m.grab * 100).toFixed(0)}%`, "grab", { icon: "target" }) +
    tile(`${maxWinX(m).toFixed(2)}×`, "max win", { hi: true, icon: "trophy" }) +
    tile(`${rtp}%`, "RTP", { icon: "bars" }) +
    `</div>` +
    `<div class="stat-grid secondary">` +
    tile(`${m.mult}×`, "clean pay", { small: true, icon: "star" }) +
    tile(`${m.bonusFactor}×`, "bonus", { small: true, icon: "medal" }) +
    tile(`${m.consolation}×`, "slip back", { small: true, icon: "back" }) +
    `</div>`;
}
...
function applyMuted(): void {
  sfx.setMuted(muted);
  soundBtn.querySelector("img")!.src = muted ? ICON.audioOff : ICON.audioOn;
